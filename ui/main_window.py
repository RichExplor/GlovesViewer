...
三栏布局:
  左栏(270px): BLE连接控制 + 数据源选择 + 录制控制 + 角度仪表盘 + 原始数据流
  中栏(弹性):  3D手部骨骼模型渲染区
  右栏(弹性):  5指弯曲度实时时序曲线
"""
...
class Ui_GlovesViewer(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.rb_ble = QtWidgets.QRadioButton("BLE 蓝牙连接")
        ...
        self.source_vbox.addWidget(self.rb_ble)
        ...
        # --- BLE连接 ---
        self.ble_box = QtWidgets.QGroupBox("BLE 连接")
        self.ble_grid = QtWidgets.QGridLayout(self.ble_box)
        self.ble_grid.addWidget(QtWidgets.QLabel("设备"), 0, 0)
        self.cb_device = QtWidgets.QComboBox()
        self.cb_device.setPlaceholderText("点击扫描...")
        self.ble_grid.addWidget(self.cb_device, 0, 1)
        self.btn_scan = QtWidgets.QPushButton("扫描")
        self.btn_scan.setStyleSheet("background-color: #17a2b8; color: white; font-size: 11px;")
        self.ble_grid.addWidget(self.btn_scan, 1, 0)
        ...
        self.ble_grid.addWidget(self.btn_connect, 1, 1)
        self.left_layout.addWidget(self.ble_box)
        ...
        # --- 角度仪表盘 ---
        self.gauge_box = QtWidgets.QGroupBox("手指角度")
        ...
        for finger in FINGER_KEYS:
            gauge = AngleGaugeWidget(finger)
            self.gauges[finger] = gauge
        ...
        for finger in FINGER_KEYS:
            ...
            plot_w.setMaximumHeight(120)

            curve = plot_w.plot(pen=pg.mkPen(color, width=1.5))
            self.curves[finger] = curve
            self.plot_fingers[finger] = plot_w
        ...
        self.lbl_status_source = QtWidgets.QLabel("数据源: BLE")
